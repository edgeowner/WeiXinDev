...
public class WxOpenKefuMessageGsonAdapter implements JsonSerializer<WxOpenKefuMessage> {

    @Override
    public JsonElement serialize(WxOpenKefuMessage message, Type typeOfSrc, JsonSerializationContext context) {
        ...
        if (WxConsts.CUSTOM_MSG_NEWS.equals(message.getMsgType())) {
            ...
            for (WxOpenKefuMessage.WxArticle article : message.getArticles()) {
        ...
        if (WxConsts.CUSTOM_MSG_MPNEWS.equals(message.getMsgType())) {
            ...
            json.addProperty("media_id", message.getOpenNewsMediaId());
