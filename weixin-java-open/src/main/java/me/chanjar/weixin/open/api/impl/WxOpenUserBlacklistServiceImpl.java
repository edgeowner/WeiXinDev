...
import me.chanjar.weixin.open.api.WxOpenUserBlacklistService;
...
public class WxOpenUserBlacklistServiceImpl implements WxOpenUserBlacklistService {
    ...
    @Override
    public WxOpenUserBlacklistGetResult getBlacklist(String nextOpenid) throws WxErrorException {
        JsonObject jsonObject = new JsonObject();
        jsonObject.addProperty("begin_openid", nextOpenid);
        String url = API_BLACKLIST_PREFIX + "/getblacklist";
        String responseContent = this.wxOpenService.execute(new SimplePostRequestExecutor(), url, jsonObject.toString());
        return WxOpenUserBlacklistGetResult.fromJson(responseContent);
    }
    ...
    public void pushToBlacklist(List<String> openidList) throws WxErrorException {
        ...
        this.wxOpenService.execute(new SimplePostRequestExecutor(), url, new Gson().toJson(map));
    }
