package me.chanjar.weixin.open.api.impl;

import com.google.gson.JsonObject;
import me.chanjar.weixin.common.bean.result.WxError;
import me.chanjar.weixin.common.bean.result.WxMediaUploadResult;
import me.chanjar.weixin.common.exception.WxErrorException;
import me.chanjar.weixin.common.util.http.MediaUploadRequestExecutor;
import me.chanjar.weixin.open.api.WxOpenKefuService;
import me.chanjar.weixin.open.api.WxOpenService;
import me.chanjar.weixin.open.bean.kefu.WxOpenKefuMessage;
import me.chanjar.weixin.open.bean.kefu.request.WxOpenKfAccountRequest;
import me.chanjar.weixin.open.bean.kefu.request.WxOpenKfSessionRequest;
import me.chanjar.weixin.open.bean.kefu.result.*;
import org.slf4j.Logger;
import org.slf4j.LoggerFactory;

import java.io.File;
import java.util.Date;

/**
 * @author Binary Wang
 */
public class WxOpenKefuServiceImpl implements WxOpenKefuService {
    private static final String API_URL_PREFIX = "https://api.weixin.qq.com/customservice";
    private static final String API_URL_PREFIX_WITH_CGI_BIN = "https://api.weixin.qq.com/cgi-bin/customservice";
    protected final Logger logger = LoggerFactory.getLogger(this.getClass());
    private final WxOpenService wxOpenService;

    public WxOpenKefuServiceImpl(WxOpenService wxOpenService) {
        this.wxOpenService = wxOpenService;
    }

    @Override
    public boolean kfAccountAdd(WxOpenKfAccountRequest request) throws WxErrorException {
        String url = API_URL_PREFIX + "/kfaccount/add";
        String responseContent = this.wxOpenService.post(url, request.toJson());
        return responseContent != null;
    }

    @Override
    public boolean kfAccountUpdate(WxOpenKfAccountRequest request) throws WxErrorException {
        String url = API_URL_PREFIX + "/kfaccount/update";
        String responseContent = this.wxOpenService.post(url, request.toJson());
        return responseContent != null;
    }

    @Override
    public boolean kfAccountDel(String kfAccount) throws WxErrorException {
        String url = API_URL_PREFIX + "/kfaccount/del?kf_account=" + kfAccount;
        String responseContent = this.wxOpenService.get(url, null);
        return responseContent != null;
    }

    @Override
    public boolean kfAccountUploadHeadImg(String kfAccount, File imgFile) throws WxErrorException {
        String url = API_URL_PREFIX + "/kfaccount/uploadheadimg?kf_account=" + kfAccount;
        WxMediaUploadResult responseContent = this.wxOpenService.execute(new MediaUploadRequestExecutor(), url, imgFile);
        return responseContent != null;
    }

    @Override
    public WxOpenKfList kfList() throws WxErrorException {
        String url = API_URL_PREFIX_WITH_CGI_BIN + "/getkflist";
        String responseContent = this.wxOpenService.get(url, null);
        return WxOpenKfList.fromJson(responseContent);
    }

    @Override
    public boolean sendKefuMessage(WxOpenKefuMessage message) throws WxErrorException {
        String url = "https://api.weixin.qq.com/cgi-bin/message/custom/send";
        String responseContent = this.wxOpenService.post(url, message.toJson());
        return responseContent != null;
    }

    @Override
    public WxOpenKfOnlineList kfOnlineList() throws WxErrorException {
        String url = API_URL_PREFIX_WITH_CGI_BIN + "/getonlinekflist";
        String responseContent = this.wxOpenService.get(url, null);
        return WxOpenKfOnlineList.fromJson(responseContent);
    }

    @Override
    public boolean kfAccountInviteWorker(WxOpenKfAccountRequest request) throws WxErrorException {
        String url = API_URL_PREFIX + "/kfaccount/inviteworker";
        String responseContent = this.wxOpenService.post(url, request.toJson());
        System.out.println(responseContent);
        return responseContent != null;
    }

    @Override
    public boolean kfSessionCreate(String openid, String kfAccount)
            throws WxErrorException {
        WxOpenKfSessionRequest request = new WxOpenKfSessionRequest(kfAccount, openid);
        String url = API_URL_PREFIX + "/kfsession/create";
        String responseContent = this.wxOpenService.post(url, request.toJson());
        return responseContent != null;
    }

    @Override
    public boolean kfSessionClose(String openid, String kfAccount)
            throws WxErrorException {
        WxOpenKfSessionRequest request = new WxOpenKfSessionRequest(kfAccount, openid);
        String url = API_URL_PREFIX + "/kfsession/close";
        String responseContent = this.wxOpenService.post(url, request.toJson());
        return responseContent != null;
    }

    @Override
    public WxOpenKfSessionGetResult kfSessionGet(String openid)
            throws WxErrorException {
        String url = API_URL_PREFIX + "/kfsession/getsession?openid=" + openid;
        String responseContent = this.wxOpenService.get(url, null);
        return WxOpenKfSessionGetResult.fromJson(responseContent);
    }

    @Override
    public WxOpenKfSessionList kfSessionList(String kfAccount)
            throws WxErrorException {
        String url = API_URL_PREFIX + "/kfsession/getsessionlist?kf_account=" + kfAccount;
        String responseContent = this.wxOpenService.get(url, null);
        return WxOpenKfSessionList.fromJson(responseContent);
    }

    @Override
    public WxOpenKfSessionWaitCaseList kfSessionGetWaitCase()
            throws WxErrorException {
        String url = API_URL_PREFIX + "/kfsession/getwaitcase";
        String responseContent = this.wxOpenService.get(url, null);
        return WxOpenKfSessionWaitCaseList.fromJson(responseContent);
    }

    @Override
    public WxOpenKfMsgList kfMsgList(Date startTime, Date endTime, Long msgId, Integer number) throws WxErrorException {
        if (number > 10000) {
            throw new WxErrorException(WxError.newBuilder().setErrorMsg("非法参数请求，每次最多查询10000条记录！").build());
        }

        if (startTime.after(endTime)) {
            throw new WxErrorException(WxError.newBuilder().setErrorMsg("起始时间不能晚于结束时间！").build());
        }

        String url = API_URL_PREFIX + "/msgrecord/getmsglist";

        JsonObject param = new JsonObject();
        param.addProperty("starttime", startTime.getTime() / 1000); //starttime	起始时间，unix时间戳
        param.addProperty("endtime", endTime.getTime() / 1000); //endtime	结束时间，unix时间戳，每次查询时段不能超过24小时
        param.addProperty("msgid", msgId); //msgid	消息id顺序从小到大，从1开始
        param.addProperty("number", number); //number	每次获取条数，最多10000条

        String responseContent = this.wxOpenService.post(url, param.toString());

        return WxOpenKfMsgList.fromJson(responseContent);
    }

    @Override
    public WxOpenKfMsgList kfMsgList(Date startTime, Date endTime) throws WxErrorException {
        int number = 10000;
        WxOpenKfMsgList result = this.kfMsgList(startTime, endTime, 1L, number);

        if (result != null && result.getNumber() == number) {
            Long msgId = result.getMsgId();
            WxOpenKfMsgList followingResult = this.kfMsgList(startTime, endTime, msgId, number);
            while (followingResult != null && followingResult.getRecords().size() > 0) {
                result.getRecords().addAll(followingResult.getRecords());
                result.setNumber(result.getNumber() + followingResult.getNumber());
                result.setMsgId(followingResult.getMsgId());
                followingResult = this.kfMsgList(startTime, endTime, followingResult.getMsgId(), number);
            }
        }

        return result;
    }

}
