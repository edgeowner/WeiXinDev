package me.chanjar.weixin.open.bean.kefu;

import me.chanjar.weixin.common.api.WxConsts;
import me.chanjar.weixin.open.bean.kefu.WxOpenKefuMessage.WxArticle;
import org.testng.Assert;
import org.testng.annotations.Test;

@Test
public class WxOpenKefuMessageTest {

    public void testTextReply() {
        WxOpenKefuMessage reply = new WxOpenKefuMessage();
    ...
    public void testTextBuild() {
        WxOpenKefuMessage reply = WxOpenKefuMessage.TEXT().toUser("OPENID").content("sfsfdsdf").build();
        Assert.assertEquals(reply.toJson(), "{\"touser\":\"OPENID\",\"msgtype\":\"text\",\"text\":{\"content\":\"sfsfdsdf\"}}");
    }
    ...
    public void testImageBuild() {
        WxOpenKefuMessage reply = WxOpenKefuMessage.IMAGE().toUser("OPENID").mediaId("MEDIA_ID").build();
        Assert.assertEquals(reply.toJson(), "{\"touser\":\"OPENID\",\"msgtype\":\"image\",\"image\":{\"media_id\":\"MEDIA_ID\"}}");
    }
    ...
    public void testVoiceBuild() {
        WxOpenKefuMessage reply = WxOpenKefuMessage.VOICE().toUser("OPENID").mediaId("MEDIA_ID").build();
        Assert.assertEquals(reply.toJson(), "{\"touser\":\"OPENID\",\"msgtype\":\"voice\",\"voice\":{\"media_id\":\"MEDIA_ID\"}}");
    }
    ...
    public void testVideoBuild() {
        WxOpenKefuMessage reply = WxOpenKefuMessage.VIDEO().toUser("OPENID").title("TITLE").mediaId("MEDIA_ID").thumbMediaId("MEDIA_ID").description("DESCRIPTION").build();
        Assert.assertEquals(reply.toJson(), "{\"touser\":\"OPENID\",\"msgtype\":\"video\",\"video\":{\"media_id\":\"MEDIA_ID\",\"thumb_media_id\":\"MEDIA_ID\",\"title\":\"TITLE\",\"description\":\"DESCRIPTION\"}}");
    }
    ...
    public void testMusicBuild() {
        WxOpenKefuMessage reply = WxOpenKefuMessage.MUSIC()
                .toUser("OPENID")
                .title("TITLE")
    ...
    public void testNewsBuild() {
        ...
        WxOpenKefuMessage reply = WxOpenKefuMessage.NEWS().toUser("OPENID").addArticle(article1).addArticle(article2).build();
